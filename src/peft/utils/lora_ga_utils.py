...
class LoraGAContextAll:
    def __init__(
        ...
        if peft_config is not None:
            ...
            self.lora_ga_config = peft_config.lora_ga_config
        else:
            self.lora_ga_config = LoraGAConfig() if lora_ga_config is None else lora_ga_config

        if self.named_grad is None:
            assert dataloader is not None or train_dataset is not None, "dataloader is  None and train_dataset is None"
            if dataloader is None and temp_set is not None:
                if isinstance(train_dataset, list):
                    temp_set = train_dataset[: self.lora_ga_config.bsz * self.lora_ga_config.iters]
                else:
                    temp_set = train_dataset.select(range(self.lora_ga_config.bsz * self.lora_ga_config.iters))
                dataloader = torch.utils.data.DataLoader(temp_set, batch_size=self.lora_ga_config.bsz)
            self.named_grad = estimate_gradient(
                model=self.model,
        ...
        self.peft_config.init_lora_weights = "lora_ga"
        self.peft_config.peft_type = "LORAGA"
